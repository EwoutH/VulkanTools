...
class ApiDumpSettings {
   public:
    ApiDumpSettings() {
        ...
        std::string env_value = GetPlatformEnvVar("APIDUMP_LOG_FILENAME");
        if (!env_value.empty()) {
        ...
        env_value = GetPlatformEnvVar("APIDUMP_OUTPUT_FORMAT");
        if (!env_value.empty()) {
